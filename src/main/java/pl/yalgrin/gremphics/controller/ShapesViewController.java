...
public class ShapesViewController extends AbstractController {
    ...
    private ShapesHolder shapesHolder;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        selectButton.setOnAction(e -> {
            ...
            shapesHolder.setDraggingEnabled(true);
            lastSelectedButton.setDisable(false);
            lastSelectedButton = selectButton;
            selectButton.setDisable(true);
        });

        shapesHolder = new ShapesHolder();
        shapesHolder.setOnMouseDragged(e -> {
            ...
            if (shape != null) {
                shapesHolder.startDrag(shape);
            }

        });
        shapesHolder.setOnMousePressed(e -> {
        ...
        shapesHolder.setOnMouseReleased(e -> {
            if (e.getButton() != MouseButton.PRIMARY) {
                return;
            }
            ...
            if (System.currentTimeMillis() - dragStartTime < 100 &&
                ...
                shapesHolder.cancelDrag();

                if (selectedShapeType == null) {
                    unbindProperties(selectedShape);
                    selectedShape = shapesHolder.getShapeAt((int) e.getX(), (int) e.getY());
    ...
    private void setupProperties(IShape shape) {
        ...
        button.setOnAction(e -> {
            shapesHolder.removeShape((Shape) shape);
